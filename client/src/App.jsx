  ...
  useEffect(() => {
    Promise.all([getCatalogue(), getOffers()])
      .then(([catalogueData, offerData]) => {
        ...
        setBasket(
          Object.fromEntries(
            Object.keys(catalogueData).map((name) => [name, 0])
          )
        );
      });
  }, []);
  ...
  return (
    <main className="page-shell">
      <section className="hero">
        ...
        <p>
          Select quantities and apply all active supermarket offers, including
          the maximum shampoo discount.
        </p>
      </section>
            ...
            {Object.entries(catalogue).map(([product, price]) => (
              <label className="product" key={product}>
                <span>
                  ...
                  <small>{money.format(price)}</small>
                </span>
        ...
        <aside className="stack">
          <section className="panel">
            ...
            <ul className="offers">
              {offers.map((offer, index) => (
                <li key={index}>{describeOffer(offer)}</li>
              ))}
            </ul>
          </section>
          ...
          <section className="panel result" aria-live="polite">
            ...
            <dl>
              <div>
                <dt>Subtotal</dt>
                <dd>{money.format(result?.subTotal ?? 0)}</dd>
              </div>
              <div>
                <dt>Discount</dt>
                <dd>−{money.format(result?.discount ?? 0)}</dd>
              </div>
              <div className="total">
                <dt>Total</dt>
                <dd>{money.format(result?.total ?? 0)}</dd>
              </div>
            </dl>
          </section>
        </aside>
...
function describeOffer(offer) {
  if (offer.type === "BUY_N_GET_M_FREE") {
    return `Buy ${offer.buy} ${offer.product}, get ${offer.free} free`;
  }

  if (offer.type === "PERCENTAGE") {
    return `${offer.percentage}% off ${offer.product}`;
  }

  if (offer.type === "BUY_N_GET_CHEAPEST_FREE") {
    return `Buy any ${offer.groupSize} shampoos, cheapest ${offer.freeCount} free`;
  }
