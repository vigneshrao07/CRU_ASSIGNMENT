...
export class BasketPricer {
  price(basket, catalogue, offers = []) {
    ...
    for (const [product, quantity] of Object.entries(items)) {
      if (!catalogue.hasProduct(product)) throw new Error(`Product '${product}' is in the basket but not in the catalogue`);
      subTotalPence += catalogue.getPricePence(product) * quantity;
    }
    const calculatedDiscount = (offers ?? []).reduce(
      (sum, offer) => sum + offer.calculateDiscountPence(items, catalogue), 0
    );
    const discountPence = Math.min(calculatedDiscount, subTotalPence);
